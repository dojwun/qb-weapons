    ...
    if CurrentWeaponData then
        if QBCore.Shared.Weapons[weapon]["name"] ~= "weapon_unarmed" and QBCore.Shared.Weapons[weapon]["ammotype"] == type:upper() then
            ...
            if total < maxAmmo then
                QBCore.Functions.Progressbar("taking_bullets", "Loading "..amount.." Rounds..", math.random(4000, 6000), false, true, {
                    disableMovement = false,
                    disableCarMovement = false,
                    disableMouse = false,
                    disableCombat = true,
                }, {}, {}, {}, function() -- Taskbar Finished
                    if QBCore.Shared.Weapons[weapon] ~= nil then
                        AddAmmoToPed(ped,weapon,amount)
                        TaskReloadWeapon(ped)
                        TriggerServerEvent("weapons:server:AddWeaponAmmo", CurrentWeaponData, total + amount, weapon)
                        TriggerServerEvent('QBCore:Server:RemoveItem', itemData.name, 1, itemData.slot)
                        TriggerEvent('inventory:client:ItemBox', QBCore.Shared.Items[itemData.name], "remove", 1)
                        TriggerEvent('QBCore:Notify', 'Added +'..amount..' rounds', "success")
                    end
                end, function()
            ...
            QBCore.Functions.Notify("Incorrect ammo for this weapon!", "error")
        end
    else
...
RegisterNetEvent('weapons:client:SetItemQuality', function(amount)
